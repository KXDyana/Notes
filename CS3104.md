...
* solution 1: implement the page table as a set of dedicated registers
    con: can only be used when page small is small
* solution 2: keep the page table in main memory and a page table base register (PTBR) points to the page table
    con: time required to access a user memory location is increased as there are two memory accesses needed to access a byte (one for page table entry, one for the byte).
#### solution 3: make use of Translation Lookaside Buffer (TLB)
* an associative high speed memory. 
* consists of two parts: a key and a value
* when the associative memory is presented with an item, the item is compared with all keys simultaneously. if the item is found, the coresponding value field is returned. the search is fast.

* the TLB contains only a few of the page able entries. when a logical adderss is generated by the CPU, its page number is presented to the TLB.
* if the page number is found, its frame number is immediately available and is used to access memory. (TLB hit)
* if the page number is not in the TLB. a memory reference to the page number must be made.
* if there is a TLB miss, we add the page number and frame number to the TLB, so that they will be found quickly on the next reference.
* TLB hit ratio: percentage of times a page number is found in TLB. determines effective memory access time (eat)
* eat = ( hit ratio x ( memory access time + TLB access time ) ) + ( (1 - hit ratio) x ( 2 x memory access time + TLB access time ) )

### attributes of page table entry
* frame number
* present/absent (whether the program is present in memory)
* protection: set to 0 if read only, set to 1 if R/W allowed
* reference: if this page has been referenced in the last clock cycle of not
* caching: enabling or disabling caching of the page. set to 1 if disabled
    * disable cacheing when we need fresh data to avoid fetching old data
* dirty: specifies whether the page has been modified or not. if modified, set to 1. avoid unnecessary write to secondary memory when the page is being replaced by another page and need to be put back to secondary memory therefore.

## hierarchical paging (multilevel paging)
most modern OS support a large logical adderss space. if so, the page table becomes excessively large. allocate a page table of this size contiguously in main memory is not a good idea
* solution: devide the page table into smaller sized. the page table itself is also paged.

## hashed page tables
* used for handling adderss spaces larger than 32 bits (or larger)
...
* each entry in the hash table refers to seevral pages rather than a single page
* a single page table entry can store the mappings for multiple physical page frames.
* useful for sparse address spaces where memory references are non contiguous and scattered throughout the adderss space

## inverted page tables
* in most OS, a separate page table is mained for each processes
* for n number of processes there will be n number of page tables
* for large proesses there would be many pages and for maintaining info about these pages, there would be too many entries in their page tables which would occupy a lot of memory
* hence memory utilization is not efficient.
### solution: inverted page tables
* has one entry for each real page (frame) of memory
* each entry consists of the virtual address of the page stored in that real memory location and the owner process
* only one page table is in the system and it has only one entry for each page of physical memory
* fields:
    1. process id
    2. page number
    3. offset
* when a memory reference occurs, part of the virtual address consisting of the process id and page number is presented to memory subsystem
* the inverted page table is searched for a match. if found, then the physical address i, offset is generated. if not, this is an illegal address access.

* pros: reduce memory usage
* cons: increased search time as the whole table might be searched for a match. difficult in implementing shared memory.

## Segmentation
* another non contiguous memory allocation technique like paging
* the process are not divided into pages. instead, divided into several modules called segments which improves the visualization for the users. 
* both secondary memory and main memory are divided into partitions of unequal sizes. 

* a logical address space is a collection of segments. each segment has a name a length. 
* the adderss specify both segment name and offset within the segment.
* user specifies each logical address by two quantities: segment name (number) and offset.

### segment table
map two dimensional adderss to physical address.
#### logical adderss:
* segment number s (index to segment table)
* segment offset d (0~segment limit)

* each entry in the segment table has a segment base and a segment limit
* segment base: starting physical address where the segment resides in
* segment limit: length of segment

## virtual memory
* purpose of memory management: to keep more processes in meomry simultaneous to allow multi programming. however, they tend to require an entire process to be in memory before it can execute, so the processes that can be loaded onto the main memory is often limited by the size of the main memory
* purpose of virtual memory: storage scheme where secondary memory can be treated as though it is a part of main memory and large processes can be stored in. only the part of the processes that is actually needed for execution will be loaded onto the actual main memory.
* pros:    
    1. program no longer contrained by the amount of physical memory.
    2. each user program could take less physical memory, more programs could be running at the same time.
    3. less I/O needed to load or swap user program into memory, each user program would run faster

## Demand Paging
* how can an executable program be loaded from disk to memory?
    1. load the entire program in physical memory at program execution time.
        cons: may not need the entire program in memory. 
    2. load pages only as they are needed (Demand Paging).

    * demand paging is similar to a paging system with swapping where processes reside in secondary memory (usually a disk)
    * when we want to execute a process, we swap it into memory.  rather than swapping the entire process into memory, we use a lazy swapper: never swaps a page into memory unless the page will be needed. here, using the term swapper is incorrect since not an entire process is swapped. in connection with demand paging we will use the term pager.